...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.init_size = opt.img_size // (2**4)         # 32/16=2
        self.linear = nn.Linear(opt.latent_dim, 128*self.init_size**2) # 1 * 128 * 2 * 2
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.output = nn.Sequential(
            nn.Linear(64*(opt.img_size//(2**4))**2, 1),
            nn.Sigmoid()
        )
...
print(discriminator(Variable(torch.randn(10,3,32,32)).cuda()).size())
...
# Load Data
###################################
data = datasets.CIFAR10('../../data/cifar10', train=True, download=True)
images = data.data.transpose([0,3,1,2])
